...
int main() {
...

  {
	...
	tp::ScheduleOptions nodeHeavy{.priority = tp::Priority::Normal,
								  .affinity = std::nullopt,
								  .concurrency = 2};

	auto euler = scope.then(
		gen,
		[&] {
		  auto h = pool.for_each(idx.begin(), idx.end(), [&](int i) {
